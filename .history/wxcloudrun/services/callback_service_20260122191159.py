...
def _validate_params(params, required):
    ...
    if missing:
        logger = logging.getLogger("wxcloudrun.callback_service")
        logger.error("Missing parameters: %s, args=%s", ", ".join(missing), dict(params))
...
def _decrypt_body(body: str, msg_signature: str, timestamp: str, nonce: str, receive_id: str):
    ...
    try:
        ...
        if ret != 0:
            logger.error("DecryptMsg failed ret=%s msg_signature=%s receive_id=%s", ret, msg_signature, receive_id)
            return None
        decrypted_dict = xmltodict.parse(decrypted_xml)
        return json.loads(json.dumps(decrypted_dict))
    except Exception as e:
        logger.exception("Decrypt or parse exception: %s", e)
        return None
...
def handle_data_callback(request):
    ...
    if request.method == "POST":
        ...
        if decrypted_json is None:
            logging.getLogger("wxcloudrun.callback_service").error(
                "Failed to decrypt data callback. args=%s body=%s", dict(request.args), post_data[:1000]
            )
...
def handle_command_callback(request):
    ...
    if request.method == "POST":
        ...
        if decrypted_json is None:
            logging.getLogger("wxcloudrun.callback_service").error(
                "Failed to decrypt command callback. args=%s body=%s", dict(request.args), post_data[:1000]
            )
            return jsonify({"code": 1, "message": "DecryptMsg fail"}), 400
        ...
        if info_type == "suite_ticket":
            ...
            if suite_ticket:
                try:
                    ...
                    logging.getLogger("wxcloudrun.callback_service").exception(
                        "Failed handling suite_ticket: %s", suite_ticket
                    )
            ...
            if auth_code:
                try:
                    ...
                    logging.getLogger("wxcloudrun.callback_service").exception(
                        "Failed to start async get_permanent_code for auth_code=%s", auth_code
                    )
        # 可扩展更多 InfoType 分支
        return "success", 200
