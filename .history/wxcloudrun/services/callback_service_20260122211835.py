...
def _validate_params(params, required):
    ...
    if missing:
        print("[callback_service] 缺少参数:", ", ".join(missing), "args=", dict(params))
...
def _verify_url(args, receive_id):
    ...
    print("[callback_service] VerifyURL 返回:", ret, "sEchoStr=", sEchoStr, "receive_id=", receive_id, "args=", dict(args))
    if ret == 0:
        return sEchoStr, 200
...
def _decrypt_body(body: str, msg_signature: str, timestamp: str, nonce: str, receive_id: str):
    ...
    try:
        ...
        print("[callback_service] 解密或解析异常:", e)
        return None
...
def handle_data_callback(request):
    ...
    if request.method == "POST":
        ...
        print("[callback_service] 数据回调收到 POST，args=", dict(request.args), "body_snippet=", post_data[:300])
        ...
        # 这里可以扩展数据事件处理逻辑
        print("[callback_service] 数据回调解密成功 receive_id=", receive_id)
        return "success", 200
...
def handle_command_callback(request):
    ...
    if request.method == "POST":
        ...
        nonce = request.args.get("nonce")
        post_data = request.data.decode("utf-8")
        print("[callback_service] 指令回调收到 POST，args=", dict(request.args), "body_snippet=", post_data[:300])
        ...
        print("[callback_service] 指令回调 InfoType=", info_type, "receive_id=", receive_id)
        if info_type == "suite_ticket":
            suite_ticket = decrypted_json.get("xml", {}).get("SuiteTicket")
            if suite_ticket:
                try:
                    save_suite_ticket(suite_ticket)
                    print("[callback_service] 已保存 suite_ticket 长度=", len(suite_ticket))
                    get_suite_access_token()
                except Exception:
                    import traceback

                    print("[callback_service] 处理 suite_ticket 出现异常")
                    traceback.print_exc()
        elif info_type == "create_auth":
            auth_code = decrypted_json.get("xml", {}).get("AuthCode")
            if auth_code:
                try:
                    print("[callback_service] 收到 AuthCode=", auth_code)
                    async_get_permanent_code(auth_code)
                except Exception:
                    import traceback

                    print("[callback_service] 异步获取永久授权码启动异常 auth_code=", auth_code)
                    traceback.print_exc()
        # 可扩展更多 InfoType 分支
        return "success", 200
