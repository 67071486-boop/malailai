...
def _fetch_suite_access_token(ticket: str) -> Tuple[str, int]:
    ...
    if data.get("errcode", 0) != 0:
        logging.getLogger("wxcloudrun.token_service").error(
            "fetch_suite_access_token failed: %s payload=%s", data, payload
        )
...
def get_corp_access_token(corp_id: str, permanent_code: str) -> Optional[str]:
    ...
    try:
        ...
        if data.get("errcode", 0) != 0:
            logging.getLogger("wxcloudrun.token_service").error(
                "get_corp_access_token failed: %s payload=%s", data, payload
            )
            return None
