...
def fetch_corp_access_token(
    ...
    *,
    ...
    client = CorpAuthApi(session=session, timeout=timeout, suite_token_provider=suite_provider, corp_token_provider=corp_provider)
    data = client.get_corp_token(suite_id, corp_id, permanent_code)
    corp_token_provider = corp_provider
    corp_token_provider.save_corp_access_token(corp_id, data.get("access_token"), int(data.get("expires_in", 7200)))
    return data
