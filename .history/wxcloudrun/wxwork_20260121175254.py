...
from wechat_official.WXBizMsgCrypt import WXBizMsgCrypt
import os
...
def callback():
    ...
    try:
        try:
            crypto = WeChatCrypto(WXWORK_TOKEN, WXWORK_ENCODING_AES_KEY, WXWORK_SUITE_ID)
        except Exception as e:
            logger.error(f"WeChatCrypto初始化失败: {e}")
