        ...
        if request.method == "GET":
            ...
            try:
                ...
                if info_type == "suite_ticket":
                    ...
                    print(f"收到 Suite Ticket: {suite_ticket}")
                    # 接收到 suite_ticket 后立即获取 suite_access_token
                    suite_access_token = get_suite_access_token()
                    if suite_access_token:
                        print(f"成功获取 suite_access_token: {suite_access_token}")
                    else:
                        logger.error("获取 suite_access_token 失败")
                elif info_type == "create_auth":
                    auth_code = root.find("AuthCode").text
                    global _auth_code
                    _auth_code = auth_code
                    print(f"收到临时授权码: {auth_code}")
                    # 先立即响应 success，后续异步处理永久授权码
