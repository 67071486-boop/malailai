...
def callback():
    ...
    try:
        try:
            print(f"[WXBizMsgCrypt参数] WXWORK_TOKEN={WXWORK_TOKEN}")
            print(
                f"[WXBizMsgCrypt参数] WXWORK_ENCODING_AES_KEY={WXWORK_ENCODING_AES_KEY}"
            )
            print(f"[WXBizMsgCrypt参数] WXWORK_CORP_ID={WXWORK_CORP_ID}")
        ...
        if request.method == "GET":
            ...
            print(
                f"[回调验证] msg_signature={msg_signature}, timestamp={timestamp}, nonce={nonce}, echostr={echostr}"
            )
            try:
                ...
                if ret == 0:
                    ...
                    print(f"[回调验证] VerifyURL失败: ret={ret}")
                    return "VerifyURL fail", 400
            except Exception as e:
                print(f"[回调验证] GET验证异常: {e}")
                if request.method == "POST":
                    # POST 请求：处理 suite_ticket 等回调
                    valid, error = validate_base_params(
                    ...
                    print(
                        f"[回调] POST参数: msg_signature={msg_signature}, timestamp={timestamp}, nonce={nonce}"
                    )
                    print(f"[回调] 加密内容: {encrypted_xml}")
                    ...
                    try:
                        ...
                        if ret != 0:
                            ...
                            return (
                                jsonify({"code": 1, "message": "DecryptMsg fail"}),
                                400,
                            )
                        ...
                        if not decrypted_xml.startswith("<"):
                            print(f"[回调] 解密结果不是XML格式: {decrypted_xml}")
                            return (
                                jsonify(
                                    {"code": 1, "message": "Invalid decrypted content"}
                                ),
                                400,
                            )
                        ...
                        if info_type == "suite_ticket":
                            ...
                            if suite_access_token:
                                print(
                                    f"[回调] suite_access_token 获取成功: {suite_access_token}"
                                )
                            else:
                            ...
                            Thread(
                                target=async_get_permanent_code, args=(auth_code,)
                            ).start()
                            return "success"
                        else:
                        ...
                        print(f"[回调] XML解析异常: {e}, 内容: {decrypted_xml}")
