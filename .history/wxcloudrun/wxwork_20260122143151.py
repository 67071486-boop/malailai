...
def callback():
    ...
    try:
        ...
        print(f"[WXBizMsgCrypt参数] WXWORK_ENCODING_AES_KEY={WXWORK_ENCODING_AES_KEY}")
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            encrypt = root.find('Encrypt').text if root.find('Encrypt') is not None else None
            if not encrypt:
                print(f"[回调] XML中未找到Encrypt字段: {encrypted_xml}")
                return jsonify({'code': 1, 'message': 'Missing Encrypt field'}), 400

            ret, sMsg = wxcpt.DecryptMsg(encrypt, msg_signature, timestamp, nonce)
            if ret != 0:
            ...
            print(f"[回调] XML解析异常: {e}, 内容: {encrypted_xml}")
