...
def callback():
    ...
    if request.method == 'GET':
        # URL 验证
        print(f"[回调验证] 收到 GET 请求，参数: {dict(request.args)}")
        ...
        # 处理回调消息
        print(f"[回调] 收到 POST 请求，参数: {dict(request.args)}")
        ...
        # 将原始XML内容转为JSON对象
        try:
            post_data_json = xmltodict.parse(post_data)
            post_data_dict = json.loads(json.dumps(post_data_json, ensure_ascii=False))
            # 只保留'xml'内部内容，无论是否有'xml'键
            if isinstance(post_data_dict, dict) and 'xml' in post_data_dict:
                post_data = post_data_dict['xml']
            else:
                post_data = post_data_dict
            print(f"[回调] POST内容已转为JSON: {post_data}")
        except Exception:
            print(f"[回调] POST内容不是XML，原始内容: {post_data}")
