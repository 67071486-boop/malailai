...
def get_permanent_code(auth_code):
    ...
    if not suite_token:
        print("获取 Suite Access Token 失败")
        return None
    ...
    try:
        ...
        if result.get('errcode') == 0:
            ...
            print(f"获取永久授权码成功: {corp_id}")
            return permanent_code, auth_corp_info
        else:
            print(f"获取永久授权码失败: {result}")
            return None
    except Exception as e:
        print(f"获取永久授权码异常: {e}")
        return None
...
def callback_data():
    ...
    try:
        ...
        print("[WXBizMsgCrypt] 初始化成功[data]")
    except Exception as e:
        print(f"[WXBizMsgCrypt] 初始化失败[data]: {e}")
        return 'Crypto init failed', 500

    if request.method == 'GET':
        print(f"[数据回调验证] 收到 GET 请求，参数: {dict(request.args)}")
        valid, error = validate_base_params(request.args, ['msg_signature', 'timestamp', 'nonce', 'echostr'])
        if not valid:
            print(f"[数据回调验证] 参数校验失败: {error}")
            return error, 400
        ...
        print(f"[数据回调验证] msg_signature={msg_signature}, timestamp={timestamp}, nonce={nonce}, echostr={echostr}")
        try:
            ret, sEchoStr = wxcpt.VerifyURL(msg_signature, timestamp, nonce, echostr)
            print(f"[数据回调验证] VerifyURL 返回: ret={ret}, sEchoStr={sEchoStr}")
            if ret == 0:
                print(f"[数据回调] 验证URL成功: {sEchoStr}")
                return sEchoStr
            else:
                print(f"[数据回调] 验证URL失败: ret={ret}")
...
def callback_command():
    """企业微信指令回调处理（如 suite_ticket、create_auth）"""
    # try:
    #     wxcpt = WXBizMsgCrypt(WXWORK_TOKEN, WXWORK_ENCODING_AES_KEY, WXWORK_CORP_ID)
    #     print("[WXBizMsgCrypt] 初始化成功[command]")
    # except Exception as e:
    #     print(f"[WXBizMsgCrypt] 初始化失败[command]: {e}")
    #     return 'Crypto init failed', 500

    if request.method == 'GET':
        print(f"[指令回调验证] 收到 GET 请求，参数: {dict(request.args)}")
        wxcpt = WXBizMsgCrypt(WXWORK_TOKEN, WXWORK_ENCODING_AES_KEY, WXWORK_CORP_ID)
        valid, error = validate_base_params(request.args, ['msg_signature', 'timestamp', 'nonce', 'echostr'])
        if not valid:
            print(f"[指令回调验证] 参数校验失败: {error}")
            return error, 400
        ...
        print(f"[指令回调验证] msg_signature={msg_signature}, timestamp={timestamp}, nonce={nonce}, echostr={echostr}")
        try:
            ret, sEchoStr = wxcpt.VerifyURL(msg_signature, timestamp, nonce, echostr)
            print(f"[指令回调验证] VerifyURL 返回: ret={ret}, sEchoStr={sEchoStr}")
            if ret == 0:
                print(f"[指令回调] 验证URL成功: {sEchoStr}")
                return sEchoStr
            else:
                print(f"[指令回调] 验证URL失败: ret={ret}")
                return 'VerifyURL failed', 400
        except Exception as e:
            print(f"[指令回调] 验证异常: {e}")
            return jsonify({'code': 1, 'message': 'VerifyURL failed'}), 500
        ...
        wxcpt = WXBizMsgCrypt(WXWORK_TOKEN, WXWORK_ENCODING_AES_KEY, WXWORK_CORP_ID)
        print(f"[指令回调] 收到 POST 请求，参数: {dict(request.args)}")
        valid, error = validate_base_params(request.args, ['msg_signature', 'timestamp', 'nonce'])
        if not valid:
            print(f"[指令回调] 参数校验失败: {error}")
            return jsonify({'code': 1, 'message': error}), 400
        ...
        print(f"[指令回调] POST参数: msg_signature={msg_signature}, timestamp={timestamp}, nonce={nonce}")
        print(f"[指令回调] 加密内容: {post_data}")

        try:
            ret, decrypted_xml = wxcpt.DecryptMsg(post_data, msg_signature, timestamp, nonce)
            if ret != 0:
                print(f"[指令回调] DecryptMsg失败: {ret}")
                return jsonify({'code': 1, 'message': 'DecryptMsg fail'}), 400

            print(f"[指令回调] 解密后的内容: {decrypted_xml}")
            ...
            if info_type == 'suite_ticket':
                ...
                def async_get_permanent_code(auth_code):
                    try:
                        ...
                        if permanent_result:
                            print("[指令回调] 永久授权码获取成功")
                        else:
                            print("[指令回调] 永久授权码获取失败")
                    except Exception as e:
                        print(f"[指令回调] 异步获取永久授权码异常: {e}")
                Thread(target=async_get_permanent_code, args=(auth_code,)).start()
            else:
                print(f"[指令回调] 其他 InfoType: {info_type}")

            return 'success'

        except Exception as e:
            print(f"[指令回调] 解密或解析异常: {e}")
            return jsonify({'code': 1, 'message': 'Decrypt or parse failed'}), 500
