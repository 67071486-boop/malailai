...
def get_suite_access_token():
    ...
    try:
        ...
        # 官方文档：没返回errcode视为成功，失败才有errcode
        if 'errcode' not in result or result.get('errcode') == 0:
            _suite_access_token = result['suite_access_token']
            # Token 有效期7200秒，提前5分钟过期
            _suite_token_expires = current_time + result.get('expires_in', 7200) - 300
...
def callback_data():
    ...
    if request.method == 'GET':
        ...
        # 动态提取ToUserName作为sReceiveId
        try:
            import re
            m = re.search(r'<ToUserName><!\[CDATA\[(.*?)\]\]></ToUserName>', post_data)
            sReceiveId = m.group(1) if m else WXWORK_SUITE_ID
            print(f"[数据回调] 动态sReceiveId: {sReceiveId}")
            wxcpt = WXBizMsgCrypt(WXWORK_TOKEN, WXWORK_ENCODING_AES_KEY, sReceiveId)
...
def callback_command():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            m = re.search(r'<ToUserName><!\[CDATA\[(.*?)\]\]></ToUserName>', post_data)
            sReceiveId = m.group(1) if m else WXWORK_SUITE_ID
            print(f"[指令回调] 动态sReceiveId: {sReceiveId}")
            wxcpt = WXBizMsgCrypt(WXWORK_TOKEN, WXWORK_ENCODING_AES_KEY, sReceiveId)
