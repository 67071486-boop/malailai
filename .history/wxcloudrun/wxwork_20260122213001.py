...
def callback_data():
    ...
    logging.getLogger("wxcloudrun.wxwork").info("Received data callback")
...
def callback_command():
    ...
    print("[callback_service] ...", flush=True)
    logging.getLogger("wxcloudrun.wxwork").info("Received command callback")
...
def oauth_login():
    ...
    if not redirect_uri:
        print("[wxwork] oauth_login 缺少 redirect_uri，请在参数或环境变量 WXWORK_OAUTH_REDIRECT 中配置")
