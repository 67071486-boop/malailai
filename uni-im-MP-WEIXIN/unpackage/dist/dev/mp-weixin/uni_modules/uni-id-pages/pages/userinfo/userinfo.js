...
const _sfc_main = {
  ...
  onLoad(e) {
    return __async(this, null, function* () {
      ...
      let res = yield uniIdCo.getAccountInfo();
    ...
    bindThirdAccount(provider) {
      return __async(this, null, function* () {
        ...
        if (this.userInfo[bindField]) {
          yield uniIdCo2["unbind" + provider]();
          yield uni_modules_uniIdPages_common_store.mutations.updateUserInfo();
        } else {
          common_vendor.index.login({
            provider: provider.toLowerCase(),
            onlyAuthorize: true,
            success: (e) => __async(this, null, function* () {
              const res = yield uniIdCo2["bind" + provider]({
                code: e.code
              });
              if (res.errCode) {
